...
// Add a File resource (simulated) to Module 3
...
$defaultcat = $DB->get_record('question_categories', ['contextid' => context_course::instance($course->id)->id, 'parent' => 0]);
if (!$defaultcat) {
    $defaultcat = new stdClass();
    $defaultcat->contextid = context_course::instance($course->id)->id;
...
$qcat = $DB->get_record('question_categories', ['contextid' => context_course::instance($course->id)->id]);
if (!$qcat) {
    $qcat = new stdClass();
    $qcat->contextid = context_course::instance($course->id)->id;
    $qcat->name = 'Top for CD511';
    $qcat->info = '';
    $qcat->sortorder = 999;
    $qcat->stamp = make_unique_id_code();
    $qcat->parent = $defaultcat->id;
    $qcat->id = $DB->insert_record('question_categories', $qcat);
} else {
    $defaultcat = $qcat;
}

// Create 10 quiz questions (mix of MCQ and True/False)
$questions = [
    ...
    ['name' => 'Q2: Flipped Classroom', 'questiontext' => 'In a flipped classroom model, students typically review content at home and engage in activities during class time.', 'qtype' => 'truefalse', 'answers' => [
    ...
    ['name' => 'Q5: Active Learning', 'questiontext' => 'Active learning strategies require students to engage with course material through discussion, problem-solving, and reflection.', 'qtype' => 'truefalse', 'answers' => [
        ['text' => 'True', 'fraction' => 1],
        ['text' => 'False', 'fraction' => 0],
    ]],
    ['name' => 'Q6: Blended Learning', 'questiontext' => 'Blended learning combines online digital media with traditional classroom methods.', 'qtype' => 'truefalse', 'answers' => [
        ['text' => 'True', 'fraction' => 1],
        ['text' => 'False', 'fraction' => 0],
    ]],
    ...
    ['name' => 'Q8: Feedback Importance', 'questiontext' => 'Timely and specific feedback is essential for student improvement and motivation.', 'qtype' => 'truefalse', 'answers' => [
        ['text' => 'True', 'fraction' => 1],
        ['text' => 'False', 'fraction' => 0],
    ]],
    ...
    ['name' => 'Q10: Course Design', 'questiontext' => 'A well-designed online course should include clear learning objectives, varied activities, and multiple assessment opportunities.', 'qtype' => 'truefalse', 'answers' => [
        ['text' => 'True', 'fraction' => 1],
        ['text' => 'False', 'fraction' => 0],
    ]],
];
...
foreach ($questions as $qdata) {
    ...
    $question->category = $defaultcat->id;
    ...
    if ($qdata['qtype'] === 'multichoice') {
        ...
        foreach ($qdata['answers'] as $idx => $adata) {
            ...
            if ($adata['fraction'] == 1) {
                $DB->set_field('question_multichoice', 'correctfeedback', 'Correct!', ['questionid' => $question->id]);
            }
        }
    } else {
        $question->qtype = 'truefalse';
        $question->id = $truefalse->save_question($question);
        // True/False answers are handled automatically by the question type
    }
